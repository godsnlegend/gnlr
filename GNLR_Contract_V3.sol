...
// File contracts/GodsNLegends.sol
...
contract GodsNLegends is IERC20, Ownable, Pausable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string constant public name = "GodsNLegends";
    string constant public symbol = "GNLR";
    uint8 constant public decimals = 18;

    uint256 private _totalSupply = 1E7 * 1E18;
    ...
    function setFeePercentage(uint8 feePercentage_) external onlyOwner {
        require(feePercentage_ <= 10, "GodsNLegends: transaction fee percentage exceeds 10");
        require(feePercentage_ >= 0, "GodsNLegends: transaction fee percentage equals 0");
    ...
    function blacklist(address[] calldata addresses, bool isBlacklist) external {
        require(hasBlacklistPermission[msg.sender], "GodsNLegends: insufficient permission");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "GodsNLegends: transfer from blacklisted address");
        require(!isBlacklisted[recipient], "GodsNLegends: transfer to blacklisted address");
        require(!isBlacklisted[tx.origin], "GodsNLegends: transfer called from blacklisted address");
